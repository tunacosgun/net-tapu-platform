...
export class SettlementWorker implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    ...
    this.logger.log('SettlementWorker started (5s poll)');
  }
  ...
  private async tick(): Promise<void> {
    ...
    try {
      ...
      if (!this.redisLock.isHealthy()) {
        this.logger.warn('SettlementWorker: skipping tick — Redis unhealthy');
        return;
      }
  ...
  private async initiateNewSettlements(): Promise<void> {
    ...
    for (const auction of endedAuctions) {
      ...
      try {
        ...
        this.logger.error(
          `Failed to initiate settlement for auction ${auction.id}: ${(err as Error).message}`,
          (err as Error).stack,
        );
      } finally {
  ...
  private async processActiveManifests(): Promise<void> {
    ...
    for (const manifest of activeManifests) {
      ...
      try {
        ...
        this.logger.error(
          `Failed to process manifest for auction ${manifest.auctionId}: ${(err as Error).message}`,
          (err as Error).stack,
        );
      } finally {
        await this.redisLock.release(lockKey, lockValue);
  ...
  private async handleManifestExpiry(manifest: SettlementManifest): Promise<void> {
    ...
    this.logger.warn(`Settlement manifest expired for auction ${manifest.auctionId}`);
